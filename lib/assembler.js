...
function evalExpr(expr, symbols, lc) {
  ...
  case 'binary': {
    let lv = evalExpr(expr.left, symbols, lc)[1];
    let rv = evalExpr(expr.right, symbols, lc)[1];
...
function pass(AST, symbols, lc) {
  ...
  for (let statement of AST.body) {
    ...
    case 'label_def': {
      let value = evalExpression(statement.expr, symbols, lc)[1];
    ...
    case 'directive': {
      ...
      if (statement.name === 'byte') {
        ...
        let lenValue = evalExpression(statement.len, symbols, lc)[1];
...
function generateCode(AST, symbols) {
  ...
  for (let statement of AST.body) {
    ...
    case 'directive': {
      ...
      case 'word': {
        for (let expr of statement.exprs) {
          let value = evalExpression(expr, symbols)[1];
      ...
      case 'res': {
        let fillByteValue;
        let lenValue = evalExpression(statement.len, symbols)[1];
        if (statement.fillByte) {
          fillByteValue = evalExpression(statement.fillByte, symbols)[1];
...
exports.Assembler = class Assembler {
  constructor(options) {
  ...
  _assembleAST(AST) {
    ...
    if (this.options.debug) { console.log('Done'); }
